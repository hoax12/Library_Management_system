...
Label2 = Label(f1, text='SEARCH BOOKS ', font=('goudy old style', 22, 'bold'), fg='black',bg='gray98')
Label2.place(x=30, y=15)

l1 = Label(f1, text='Book ID: ', font=('goudy_old_style 14 bold'), bd=2, fg='black',
               bg='white').place(x=40, y=75)
e1 = Entry(f1, width=30, bd=10, bg='orange', font=('times new roman', 15), fg='black', textvariable=sid1,
               borderwidth=2, relief=SOLID).place(x=140, y=75)
...
def search():
     ...
     try:
        if k!="":
         ...
         cur = con.cursor()
         cur.execute("select * from books_info where id=%s",(k.capitalize()))
         g = cur.fetchall()
         if len(g) !=0:
               for row in g:
...
b1 = Button(f1, text='Search', bg='orange', font='eras_medium_itc 14 bold', width=8, bd=2, borderwidth=2,relief=SOLID, command=search).place(x=480, y=65)
b2 = Button(f1, text='Back', bg='orange', font='eras_medium_itc 14 bold', width=8, bd=2, borderwidth=2,relief=SOLID, command=back).place(x=250, y=420)
