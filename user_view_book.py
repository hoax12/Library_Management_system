...
Label1 = Label(f1, text='ALL AVAILABLE BOOKS ', font=('goudy old style', 22, 'bold'), fg='black',
                            bg='gray98')
Label1.place(x=24, y=28)

my_tree = ttk.Treeview(f1)
my_tree['columns'] = ("Book ID", "Title", "Author", "Genre", "Copies")

my_tree.column("#0", width=0, stretch=NO)
my_tree.column("Book ID", anchor=W, width=50)
my_tree.column("Title", anchor=CENTER, width=160)
...
cur = con.cursor()
cur.execute("select id,name,author,genre,copies from books_info")
g = cur.fetchall()
if len(g) != 0:
    for row in g:
