...
def forget_password():
    ...
    def reset_password():
        if securityquescombo.get() == 'Select' or answerforgetEntry.get() == '' or newpassEntry.get() == '':
            ...
            try:
                ...
                cur = con.cursor()
                cur.execute('select * from employee2 where email=%s and question=%s and answer=%s',
                ...
                if row == None:
                    ...
                    cur.execute('update employee2 set password=%s where email=%s',
    ...
    if mailentry.get() == '':
        ...
        try:
            con = pymysql.connect(host='localhost', user='root', password='Sid@1234', database='myprojectdb')
            cur = con.cursor()
            cur.execute('select * from employee2 where email=%s', mailentry.get())
            row = cur.fetchone()
            if row == None:
...
def signin():
    if mailentry.get() == '' or passentry.get() == '':
        ...
        try:
            con = pymysql.connect(host='localhost', user='root', password='Sid@1234', database='myprojectdb')
            cur = con.cursor()
            cur.execute('select * from employee2 where email=%s and password=%s', (mailentry.get(), passentry.get()))
            row = cur.fetchone()
            if row == None:
...
mailLabel = Label(frame, text='Email', font=('goudy old style', 22, 'bold'), bg='white', fg='black')
mailLabel.place(x=220, y=32)
mailentry = Entry(frame, font=('arial', 20,),fg='black', bg='white',borderwidth=2,relief=SOLID)
...
pwd=StringVar()
passentry = Entry(frame,textvariable=pwd, font=('arial', 20,),fg='black', bg='white',borderwidth=2,relief=SOLID,show = '*')
passentry.place(x=220, y=160)
regbutton = Button(frame, text='Register New Account?', font=('arial', 11,'bold'), bd=0, fg='black', bg='white',
...
loginbutton2.place(x=440, y=250)
