...
Label1 = Label(f1, text='BOOK REQUESTS ', font=('goudy old style', 22, 'bold'), fg='black',
                            bg='gray98')
Label1.place(x=24, y=28)

my_tree = ttk.Treeview(f1)
my_tree['columns'] = ("Std_ID", "Std_Name", "Title", "Author", "Date", "Comments")

my_tree.column("#0", width=0, stretch=NO)
my_tree.column("Std_ID", anchor=W, width=50)
my_tree.column("Std_Name", anchor=CENTER, width=100)
my_tree.column("Title", anchor=W, width=140)
my_tree.column("Author", anchor=W, width=140)
my_tree.column("Date", anchor=W, width=80)
my_tree.column("Comments", anchor=W, width=80)
my_tree.heading("#0", text="", anchor=W)
my_tree.heading("Std_ID", text="Std_ID", anchor=W)
...
cur = con.cursor()
cur.execute("select * from request_book")
g = cur.fetchall()
if len(g) != 0:
    for row in g:
